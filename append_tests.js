...
describe('API endpoint wrappers', () => {
  ...
  describe('agents endpoints', () => {
    ...
    it('create', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith('http://localhost:8000/agents', expect.objectContaining({ method: 'POST', body: JSON.stringify({ name: 'a1', system_prompt: 'p' }) }));
    });
    ...
    it('update', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith('http://localhost:8000/agents/a1', expect.objectContaining({ method: 'PUT', body: JSON.stringify({ name: 'a2' }) }));
    });
  ...
  describe('teams endpoints', () => {
    ...
    it('create', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith('http://localhost:8000/teams', expect.objectContaining({ method: 'POST', body: JSON.stringify({ name: 't1' }) }));
    });
    ...
    it('update', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith('http://localhost:8000/teams/t1', expect.objectContaining({ method: 'PUT', body: JSON.stringify({ name: 't2' }) }));
    });
